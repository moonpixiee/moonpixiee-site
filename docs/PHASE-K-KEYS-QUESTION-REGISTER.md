...
3. **Repo / Supabase audit — Phase K discovery (PENDING Luna's go; touches infra outside this repo).** Confirm the `velvet-muse-studio` repo's actual name and location, and flag any other repos touching the same Supabase project. **Note:** `moonpixiee-site` contains no Supabase config or auth (verified 3 Aug — static Astro site); `velvet-muse-studio` is not in this working directory. This audit is Phase K discovery against a separate ecosystem, to run only when authorized — and per §0 the dependency map has Phase K schema/user-facing work waiting on Phase R cutover (the Phase R stamp is still pending the Gate 2 re-walk). **A shared, already-live Supabase project is an unverified assumption at this point, not fact.** The "no Supabase project configuration yet" line below correctly refers to THIS repo (`moonpixiee-site`), which has none; Velvet's Supabase reality lives in `velvet-muse-studio` and is invisible from here. Replacing that assumption with truth is precisely the audit's job — to run in (or pointed at) `velvet-muse-studio`, with `list_organizations` / `list_projects` via the Supabase MCP, on Luna's go. **→ AUDIT RUN 3 Aug 2026 — see "Phase K Discovery — Identity Audit" below: exactly one project (`velvet-muse-studio`, ref `pcgzhlxjcjdeweygdzci`), **INACTIVE/paused**, no separate shared project; its schema remains unverified because the paused DB is unreachable (restore required).**

---

## Phase K Discovery — Identity Audit (report only; 3 Aug 2026)

Run from the `moonpixiee-site` session on Luna's go. **Report only — no auth code, no schema changes, nothing built.** Supabase MCP used for read-only discovery (`list_organizations`, `list_projects`, attempted `list_tables`). This section replaces the "shared, already-live Supabase project" assumption with what was actually seen, and marks honestly what could not be seen.

### A. Verified (seen via Supabase MCP)
- **One organization:** `moonpixiee llc` (`pktkagixhyecovgwdetx`).
- **One project:** `velvet-muse-studio` (ref `pcgzhlxjcjdeweygdzci`), region **`ca-central-1`** (Canada), Postgres **17.6**, created **2026-07-15**.
- **Project status: `INACTIVE` (paused).** The "already-live" assumption is **corrected** — the project is paused, not currently live. There is **no separate "House" Supabase project**; if House and Velvet share identity, it is *this one project*.
- **`moonpixiee-site` (this repo):** re-verified — **zero** Supabase/auth/`user_id` references in `src`/`public`/`tools`/config; sole dependency is `astro`. Static site. It is **not** an identity surface today.

### B. Blocked (could NOT be seen — must be unblocked before any schema-specific plan)
- **The Velvet schema** — tables, auth providers, user/profile tables, which tables reference `auth.users`, RLS policies, storage buckets. `list_tables` returned a **connection timeout**: a paused project's database is unreachable. **Reading the schema requires the project to be restored/unpaused** (Luna's explicit go — not done here; restoring wakes infra and is a state change, outside "report only").
- **Real users vs Luna's test account; which records must survive migration** (Muses, scenes, outputs, images, Codex, projects, preferences, storage objects) — needs the active DB **or** the code.
- **Velvet application code** — `velvet-muse-studio` is **not reachable from this session** (not a sibling, not on this machine's home paths). Auth implementation, client config, and migrations can only be audited **in / pointed at that repo**.

**Unblock path (pick one or both, on Luna's go):** (a) restore `velvet-muse-studio`, then re-run `list_tables` / `list_migrations` / `get_advisors(security)` read-only to capture the true schema, auth config, RLS gaps, and record inventory; (b) run the code audit in a session that can see `velvet-muse-studio`. **Until then, the schema is unverified and the migration sequence below is gated on Step 0.**

### C. Proposed canonical architecture (endorses the ratified model; for ruling, not built)
Consistent with **"One House. One Address."** (Constitution v4.3; one root forever, wings as subdomains, one key across `*.housesofluna.com`):
- **`housesofluna.com`** — the House (this static Astro site: Threshold + Grand Hall). Live on Netlify.
- **`/velvet`** — the ceremonial entrance *in* the House (a door/route that leads to Velvet). Presentation only; not a separate root.
- **`velvet.housesofluna.com`** — the Velvet app (the `velvet-muse-studio` codebase), a **subdomain**, per ratified law. (Migration target — its current deploy domain is unverified from here.)
- **One Supabase Auth identity** — the single project's `auth.users` is the House's identity backbone; House and Velvet share it. **Rename note:** the project is named `velvet-muse-studio` but would be serving the whole House — a cosmetic rename (e.g. `houses-of-luna`) is optional; the ref (`pcgzhlxjcjdeweygdzci`) is what matters and is stable.
- **The load-bearing session detail (KQ-1 constraint made concrete):** Supabase Auth stores sessions **host-only by default** (SDK localStorage, or `@supabase/ssr` host cookies). To make one key hold across `housesofluna.com` and `velvet.housesofluna.com`, the session must live in a **cookie scoped to `domain=.housesofluna.com`** (a cookie-storage adapter on the House's static client + matching cookie options in the Velvet app), **or** a **custom Supabase auth domain** (`auth.housesofluna.com`, a Supabase paid feature). This is a mechanism/schema constraint from day one, **not a later toggle** — and it is the same for whichever KQ-1 mechanism (magic link / passkey / password) is ruled. Auth **Site URL + redirect URLs** must list both hosts.
- **No better alternative found.** The proposed model is the correct realization of the ratified domain law; the only genuine fork to rule is *domain-scoped cookie via SDK config* vs *custom auth domain* (tradeoff: engineering simplicity vs cleanest cross-subdomain cookie/CORS + a paid Supabase feature).

### D. Privacy constraints as schema shape (design, not dependent on current schema)
**K-USE-1 — sealed Becoming pages.** Honest layer-by-layer analysis of what "sealed" can guarantee:
- **Postgres at-rest encryption (Supabase default, disk-level AES)** protects against physical disk/backup theft **but not the operator** — the operator holds the keys and can query the DB. At-rest encryption alone does **not** deliver "the House holds the writing but does not read it."
- **The only layer that delivers operator-sealing is client-side encryption:** the resident's key (derived from their passphrase/passkey, **never sent to the server**) encrypts page content in the browser; the DB stores **ciphertext only**. Then Supabase/Luna hold ciphertext they cannot read.
- **Honest limits, all written down:** client-side encryption means **metadata still shows** (that a page exists, when written/revisited, length) — which is exactly and only what K-USE-2 is allowed to use; transport is TLS; **backups store ciphertext** (good); **logs must never carry plaintext**; DB/operator admin sees ciphertext. **Recovery tension:** if the encryption key is client-held and the resident loses it, the content is unrecoverable — so "Lost Your Key?" (KQ-1) must **re-derive the same encryption key** from the recovery factor, or the seal and recovery are in direct conflict. This tension is a **required KQ-1 + KQ-2 ruling**, not an implementation footnote.
- **Schema consequence:** sealed content is an **opaque ciphertext blob**, stored **separately** from its metadata (`written_at`, `last_revisited_at`, `prompt_id`, optional length) — the metadata is never the content.

**K-USE-2 — metadata-only delivery, structurally unavailable.** The delivery path selects letters from **metadata only** (counts, dates, revisit events, absence duration, thresholds/moon dates). "Structurally unavailable, not merely unused" is achieved by construction: sealed content is **ciphertext with no server-side key**, so even a function that read the column gets opaque bytes; and content is **not a column, join, index, or function input** on any delivery-path table (enforced by table separation + RLS). Content-aware delivery would require an explicit, revocable, resident-held opt-in as its own amendment (per the ruling in §K-USE-2).

### E. Proposed migration sequence (gated; report only)
**Step 0 — Verification (unblocks everything; Luna's go).** Restore the project and/or open `velvet-muse-studio`; capture the true schema, auth providers, RLS, storage, and the **record inventory that must survive** (Muses, scenes, outputs, images, Codex, projects, preferences, storage objects), and **real-users-vs-test-account** reality. *Nothing below is finalized until Step 0 replaces assumptions with facts.*
**Step 1 — Rulings first (KQ-1 mechanism, KQ-2 charter).** The register's own ordering: mechanism + memory charter are the constitution of the phase; the sealed-page/recovery tension in (D) is part of this ruling.
**Step 2 — Identity backbone.** Adopt the single project's `auth.users` as the House identity; configure **domain-scoped sessions** across `*.housesofluna.com` (cookie `domain=.housesofluna.com` or custom auth domain); set Auth Site/redirect URLs for both hosts.
**Step 3 — Schema for the charter,** privacy constraints baked in from the first migration: sealed content (ciphertext) separated from metadata; per-resident RLS; the metadata-only delivery path; KQ-9's ownership model with **no follower/popularity/ranking fields** (their absence is constitutional).
**Step 4 — Surfaces.** House `/velvet` ceremonial entrance; Velvet app moved to `velvet.housesofluna.com`; Your Room debut (KQ-5) gated on Phase R cutover (met) + furnishing.
**Step 5 — Commerce interlock (KQ-6)** and legacy Gumroad buyers, per that question's ruling.

*This is a proposal for ruling, not a decision. Per this register: questions before decisions, decisions before code.*
