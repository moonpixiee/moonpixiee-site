...
Batched from Luna's post-walk defect list after the stamp; **Phase R stays closed** — these are patina, not reopened acceptance. Approved on Luna's dev-preview walk ("the seam reads as one House"); a real-device phone re-walk of the three spots is pending. Committed `fix(phase-r): DR-PS-1..3`.
